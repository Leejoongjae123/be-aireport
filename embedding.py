...
def extract_text_and_images_from_pdf(pdf_path: str, output_folder: str) -> Tuple[List[Dict], List[str]]:
    """
    PyMuPDF를 사용하여 PDF에서 텍스트를 추출하고, 각 페이지를 하나의 이미지로 렌더링합니다.
    ...
    for page_num in range(len(doc)):
        ...
        # 페이지 전체를 하나의 이미지로 렌더링
        # DPI를 높이면 고화질 이미지를 얻을 수 있으나 파일 크기 증가
        zoom = 2.0  # 2배 확대 (72 DPI -> 144 DPI)
...
def process_single_folder_by_name(
    ...
    if collection_name is None:
        # ChromaDB는 한글을 지원하지 않으므로 MD5 해시 사용
        import hashlib
        safe_name = hashlib.md5(folder_name.encode()).hexdigest()[:16]
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("1단계: 멀티모달 임베딩 처리")
    print("=" * 60)
    retriever = process_single_folder_by_name(folder_name)
    
    print("\n" + "=" * 60)
    print("2단계: procedure.json 기반 retrieval")
    print("=" * 60)
    ...
    # output 폴더 경로
    output_dir = f"./data/{folder_name}/output"
    ...
    print("✅ 완료!")
    print("=" * 60)
    print(f"처리된 subsection: {summary['processed']}개")
